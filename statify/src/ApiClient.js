...
class SpotifyAPI{
    ...
    async GetArtist(ArtistId, toKen ){ //this is a function that your client has, this function when called will make an api call to the spotify api 

        try {
            const response = await this.client.get('/v1/artists/'+ ArtistId, {
                headers: {
                    Accept: 'application/json',
                    Authorization: 'Bearer ' + toKen,
                    'Content-Type': 'application/json',
